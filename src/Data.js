
// function Data() {
//     return {
//         call_to_action: '提供最好最好、最全面、最个性化、最体贴的服务',
//         about_us: '致知力行科技有限公司是一家专注于用户个人和特殊群体的健康管理和营养干预的智能科技公司，基于致知力行构建的包括中医药保健品和用户健康数据在内的营养健康大数据资源体系，通过致知力行的智能推荐和采购中台，结合线上和线下的咨询诊疗服务，为用户个人和特殊群体量身打造专属的数字化营养健康方案，提供个性化、细致化，智能化、动态化、高品质、高效率、低采购成本、便捷化的保健品订阅服务'
//     }
// }
// var data = {
//     call_to_action: '提供最好最好、最全面、最个性化、最体贴的服务',
//     about_us: '致知力行科技有限公司是一家专注于用户个人和特殊群体的健康管理和营养干预的智能科技公司，基于致知力行构建的包括中医药保健品和用户健康数据在内的营养健康大数据资源体系，通过致知力行的智能推荐和采购中台，结合线上和线下的咨询诊疗服务，为用户个人和特殊群体量身打造专属的数字化营养健康方案，提供个性化、细致化，智能化、动态化、高品质、高效率、低采购成本、便捷化的保健品订阅服务'
// }

export const data = {
    navigation: {
        aboutus: {
            id: 'about-us',
            text: '关于我们'
        },
        product: {
            id: 'product',
            text: '了解产品'
        },
        update: {
            id: 'update',
            text: '最新动向'
        },
        team: {
            id: 'team',
            text: '团队介绍'
        },
        contactus: {
            id: 'contact-us',
            text: '联系我们'
        },
    },
    call_to_action: '提供最好最好、最全面、最个性化、最体贴的服务',
    about_us: '致知力行科技有限公司是一家专注于用户个人和特殊群体的健康管理和营养干预的智能科技公司，基于致知力行构建的包括中医药保健品和用户健康数据在内的营养健康大数据资源体系，通过致知力行的智能推荐和采购中台，结合线上和线下的咨询诊疗服务，为用户个人和特殊群体量身打造专属的数字化营养健康方案，提供个性化、细致化，智能化、动态化、高品质、高效率、低采购成本、便捷化的保健品订阅服务',
    about_us_card: [
        {
            image: '/images/icon/Mission@2x.png',
            header: '使命',
            text: '激励健康生活模式，为健康赋能。',
        },
        {
            image: '/images/icon/Value@2x.png',
            header: '价值',
            text: '开放、信任、正直、创新、行动',
        },
        {
            image: '/images/icon/Hopes@2x.png',
            header: '愿景',
            text: '创建一个将健康视为权利而非奢侈品的世界。',
        }
    ],
    product_view: [
        {
            name: '四叶物语',
            ename: 'iClover',
            icon: '/images/logo/Yiwei.png',
            description: '以自然之名, 物营养之语。 AI驱动的个性化营养健康解决方案， 懂得年轻人健康话语的贴心小伙伴',
            image: '/images/slider/Slider1.png',
            linkText: '了解更多',
            text: '四叶物语是致知力行旗下 ICLOVER 健康生活品牌的子系列，基于营养数据图谱和用户健康数据在内的营养健康大数据资源体系和用户行为数据，通过 智能推荐系统 和 采购中台，为用户个人和特殊群体量身打造专属的数字化营养健康方案，提供个性化、智能化、动态化、多元化、高品质、高效率、低采购成本、便捷化的营养健康产品订阅。',
            feature: [
                {
                    image: '/images/icon/Cost@2x.png',
                    text: '直接对接厂商 降低采购成本',
                },
                {
                    image: '/images/icon/Health@2x.png',
                    text: '动态追踪用户健康数据变化',
                },
                {
                    image: '/images/icon/Quality@2x.png',
                    text: '甄选国内外一流保健品供应商',
                },
                {
                    image: '/images/icon/Customization@2x.png',
                    text: '定制化方案推荐',
                },
                {
                    image: '/images/icon/Needs@2x.png',
                    text: '满足不同保养需求',
                },
                {
                    image: '/images/icon/Data@2x.png',
                    text: '上门诊疗数据采集',
                },
                {
                    image: '/images/icon/Algorithm@2x.png',
                    text: '人工智能算法推荐',
                },
                {
                    image: '/images/icon/Speed@2x.png',
                    text: '全球化物流配送',
                },
            ],
        },
        {
            name: '四叶一味',
            ename: 'iTaste',
            icon: '/images/logo/Yiwei.png',
            description: '心悟灵芽，静虑修心。 AI驱动的个性化精神健康解决方案， 懂得年轻人健康话语的贴心小伙伴。',
            image: '/images/slider/Slider1.png',
            text: '四叶一味是 致知力行旗下 ICLOVER 健康生活品牌的子系列，将冥想(meditation)概念与中国禅修、太极等修身养性的本土传统文化相结合, 打造有中国文化特色的泛冥想品牌（涵盖瑜伽，禅修，太极)，开发周边产品，为用户个人量身打造专属的减压养生方案。',
            linkText: '了解更多',
            feature: [
                {
                    image: '/images/icon/Cost@2x.png',
                    text: '直接对接厂商 降低采购成本',
                },
                {
                    image: '/images/icon/Health@2x.png',
                    text: '动态追踪用户健康数据变化',
                },
                {
                    image: '/images/icon/Quality@2x.png',
                    text: '甄选国内外一流保健品供应商',
                },
                {
                    image: '/images/icon/Customization@2x.png',
                    text: '定制化方案推荐',
                },
                {
                    image: '/images/icon/Needs@2x.png',
                    text: '满足不同保养需求',
                },
                {
                    image: '/images/icon/Data@2x.png',
                    text: '上门诊疗数据采集',
                },
                {
                    image: '/images/icon/Algorithm@2x.png',
                    text: '人工智能算法推荐',
                },
                {
                    image: '/images/icon/Speed@2x.png',
                    text: '全球化物流配送',
                },
            ],
        }
    ],
    product_update: [
        {
            info: '2020年5月24日',
            link: 'https://google.com',
            header: '抗击疫情，致知力行与您同在',
            text: '直接对接厂商降低采购成本，直接对接厂商降低采购成本，直接对接厂商降低采购成本，直接对接厂商降低采购成本，直接对接厂商降低采购成本，直接对接厂商降低采购成本，直接对接厂商降低采购成本。',
        },
        {
            info: '2020年5月24日',
            link: 'https://google.com',
            header: '抗击疫情，致知力行与您同在',
            text: '直接对接厂商降低采购成本，直接对接厂商降低采购成本，直接对接厂商降低采购成本，直接对接厂商降低采购成本，直接对接厂商降低采购成本，直接对接厂商降低采购成本，直接对接厂商降低采购成本。',
        },
        {
            info: '2020年5月24日',
            link: 'https://google.com',
            header: '抗击疫情，致知力行与您同在',
            text: '直接对接厂商降低采购成本，直接对接厂商降低采购成本，直接对接厂商降低采购成本，直接对接厂商降低采购成本，直接对接厂商降低采购成本，直接对接厂商降低采购成本，直接对接厂商降低采购成本。',
        }
    ],
    media_update: [
        {
            img: '/images/slider/Slider1.png',
            link: 'https://google.com',
            header: '抗击疫情，致知力行与您同在',
            text: '直接对接厂商降低采购成本，直接对接厂商降低采购成本，直接对接厂商降低采购成本，直接对接厂商降低采购成本，直接对接厂商降低采购成本，直接对接厂商降低采购成本，直接对接厂商降低采购成本。',
        },
        {
            img: '/images/slider/Slider1.png',
            link: 'https://google.com',
            header: '抗击疫情，致知力行与您同在',
            text: '直接对接厂商降低采购成本，直接对接厂商降低采购成本，直接对接厂商降低采购成本，直接对接厂商降低采购成本，直接对接厂商降低采购成本，直接对接厂商降低采购成本，直接对接厂商降低采购成本。',
        },
        {
            img: '/images/slider/Slider1.png',
            link: 'https://google.com',
            header: '抗击疫情，致知力行与您同在',
            text: '直接对接厂商降低采购成本，直接对接厂商降低采购成本，直接对接厂商降低采购成本，直接对接厂商降低采购成本，直接对接厂商降低采购成本，直接对接厂商降低采购成本，直接对接厂商降低采购成本。',
        }
    ],
    contact: [
        {
            info: '中国',
            text: 'hi@zhizhiaction.com',
        },
        {
            info: '加拿大',
            text: 'hi@zhizhiaction.com',
        },
    ],
    customer: [
        {
            name: '翠西 Tracey',
            title: '预产妈妈',
            description: ' 胎儿不同阶段的生长发育需要不同的营养素。充足全面的孕期营养、合理的体重增长才能使宝宝更健康，妈妈分娩更顺利，产后恢复更理想。',
            image: '/images/customer/Pregnant.png',
            feature: [
                {
                    image: '/images/icon/Pills.png',
                    text: '均衡饮食',
                },
                {
                    image: '/images/icon/Fishoil.png',
                    text: '深海鱼类',
                },
                {
                    image: '/images/icon/Vitamin.png',
                    text: '综合维生素',
                },
            ]
        },
        {
            name: '程序员1',
            title: '程序员',
            description: '程序员程序员',
            image: '/images/customer/Programmer.jpeg',
            feature: [
                {
                    image: '/images/icon/Pills.png',
                    text: '均衡饮食',
                },
                {
                    image: '/images/icon/Fishoil.png',
                    text: '深海鱼类',
                },
                {
                    image: '/images/icon/Vitamin.png',
                    text: '综合维生素',
                },
            ]
        },
        {
            name: '久坐上班族2',
            title: '久坐上班族',
            description: '久坐上班族',
            image: '/images/customer/Female.jpg',
            feature: [
                {
                    image: '/images/icon/Pills.png',
                    text: '均衡饮食',
                },
                {
                    image: '/images/icon/Fishoil.png',
                    text: '深海鱼类',
                },
                {
                    image: '/images/icon/Vitamin.png',
                    text: '综合维生素',
                },
            ]
        },
    ],
    subscription: '我们还在研发当中，订阅我们的产品动态以第一时间获得最新消息！',
    connection: [
        './images/logo/36Kr.png',
        './images/logo/Hourentang.png',
        './images/logo/Tongrentang.png',
    ],
    team: '我们是一群来自中国和硅谷的小伙伴，为了大家的健康，我们准备搞点事情。',
}